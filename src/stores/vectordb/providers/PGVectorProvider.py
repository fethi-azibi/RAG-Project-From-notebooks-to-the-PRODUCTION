...
#     def __init__(self, db_client, default_vector_size: int=512, distance_method: str=None, 
...
#                 )
#                 records = await session.execute(list_tbl, {"prefix": self.pg_vector_table_prefix})
...
#                 self.logger.info(f"START: Creating index on table {collection_name}...")
...
#         if not is_collection_existed:
#             self.logger.error(f"Can not search records from non-existed collection: {collection_name}")
#             return False
...
#                 search_sql = sql_text(
#                     f'SELECT {PgVectorTableSchemeEnums.TEXT.value}, 1 - ({PgVectorTableSchemeEnums.VECTOR.value} <=> :vector) as score'
...
class PGVectorProvider(VectorDBInterface):
    ...
    def __init__(self, db_client, default_vector_size: int = 512,
